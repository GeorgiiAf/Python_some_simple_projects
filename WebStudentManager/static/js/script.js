...
function editStudent(studentId) {
    ...
    surnameInput.className = 'edit-input';
    ...
    cancelButton.onclick = () => cancelEdit(studentId, row, nameValue, surnameValue);

    // Скрываем существующие кнопки
...
function cancelEdit(studentId, row, originalName, originalSurname) {
    ...
    actionButtons.innerHTML = `
        <button class="edit-btn" onclick="editStudent('${studentId}')">Редактировать</button>
        <button class="delete-btn" onclick="deleteStudent('${studentId}')">Удалить</button>
    `;
...
// Обновленная функция viewStudentDetails
function viewStudentDetails(studentId) {
    fetch(`/get_grades/${studentId}`)
        .then(response => {
